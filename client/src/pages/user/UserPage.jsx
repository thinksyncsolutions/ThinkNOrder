...
const UserPage = () => {
  ...
  console.log("Menu sections for user page:", sections); // Debug log
  console.log("Restaurant ID:", restaurantId, "Branch ID:", branchId); // Debug log

  if (loading)
    return <div className="text-center py-20 text-xl">Loading delicious menu...</div>;

  if (error)
    return <div className="text-center py-20 text-red-500">{error.message} Seriously</div>;
    ...
    <div className="bg-gray-50 min-h-screen">
      ...
      <div className="bg-white shadow-sm sticky top-0 z-10">
        <div className="max-w-5xl mx-auto px-4 py-4">
          ...
          <div className="flex overflow-x-auto pb-2 gap-2 no-scrollbar mt-4">
            ...
            <div className="grid gap-6">
              {section.items?.length > 0 ? (
                section.items.map((item) => (
                  <div
                    key={item._id}
                    className="bg-white p-4 rounded-xl shadow-sm flex justify-between items-start border border-gray-100 hover:shadow-md transition"
                  >
