...
const BillManagement = ({
  ...
  const { tableOrders, lastSettledBill, loading, sessionClosing, error } = useSelector((state) => state.order);
  const {branchName, restaurantName} = useSelector((state) => state.auth.user);
  const [paymentMode, setPaymentMode] = useState("Cash");
  const [isCartOpen, setIsCartOpen] = useState(false);

  console.log("Current Table Orders:", lastSettledBill); // Debug log
...
const handleSettleAndPrint = async () => {
  try {
    ...
    // 2. Close Session and get the exact figures from the Backend
    const result = await dispatch(
      closeSession({
        placeId: tableId,
        paymentMode,
    ...
    const snapshot = {
      ...
      customerName,
      customerPhone,
      paymentMode: result.session.paymentMode,
      subtotal: result.session.subtotal,   // Use backend subtotal
      tax: result.session.tax,             // Use backend tax
      grandTotal: result.session.billAmount // Use backend billAmount
    };
